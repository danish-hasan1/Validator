...
// ── ResultBlock — expanded state keyed to id so it persists across streams ─
export function ResultBlock({ label, content, id, busy, extraActions, onCopy, copied }) {
  // Use ref-map so expand state survives re-renders as content streams in
  const expandedRef = useRef({});
  const [, forceUpdate] = useState(0);
  const expanded = expandedRef.current[id] || false;
  const setExpanded = (v) => { expandedRef.current[id]=v; forceUpdate(n=>n+1); };

  if (!content && !busy) return null;
  const isLong = content && content.length > 2500;

  return (
    <div className="clay-card" style={{ marginTop:20, overflow:'hidden', padding:0 }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'12px 18px', borderBottom:'1px solid #e8ecf5', background:'#fafbff' }}>
        <span style={{ fontSize:11, fontWeight:800, color:'#5b6af5', letterSpacing:'1px', textTransform:'uppercase' }}>{label}</span>
