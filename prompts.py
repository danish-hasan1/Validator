PROMPTS = {
    "p1": lambda jd: f"""You are the original hiring decision-maker who authored this job description, combined with a senior talent evaluator.
...
{jd}""",
    ...
    "p2": lambda jdAnalysis: f"""Convert the JD analysis below into a structured, weighted evaluation framework.
...
{jdAnalysis}""",
    ...
    "p3": lambda jdAnalysis, skillMatrix, cv: f"""You are a senior recruitment professional evaluating a candidate.
...
{cv}""",
    ...
    "p4": lambda cvEval: f"""Answer strictly as the hiring decision-maker. Direct and decisive.
...
{cvEval}""",
    ...
    "p5": lambda jdAnalysis, skillMatrix, cvEval: f"""Generate a targeted clarification questionnaire for this candidate.
...
CV Evaluation: {cvEval}""",
    ...
    "p6": lambda jdAnalysis: f"""Convert this role analysis into LinkedIn Recruiter search parameters.
...
{jdAnalysis}""",
    ...
    "compare": lambda jdName, candidates: f"""You are a senior hiring panel facilitator comparing {len(candidates)} candidates for: {jdName}

{"\n\n---\n\n".join([f"### {i+1}. {c['name']}\nScore: {c['score']}/100 | {c['recommendation']}\n\n{c['eval']}" for i, c in enumerate(candidates)])}

## 1. Ranking & Recommendation Order
Rank all {len(candidates)} candidates with clear rationale. One decisive line per candidate.
...
First-choice with rationale. Contingency if they decline.""",
    ...
    "outreachMsg": lambda jdName, candName, candTitle, outreach_type, customNote: f"""You are a senior recruiter writing a personalised {'WhatsApp' if outreach_type == 'whatsapp' else 'email'} outreach message.

Role: {jdName}
Candidate: {candName}{f' ({candTitle})' if candTitle else ''}
{f'Recruiter note: {customNote}' if customNote else ''}

Write a {'brief, warm WhatsApp message (3–4 sentences max, conversational, no bullet points)' if outreach_type == 'whatsapp' else 'professional but warm email (subject line + body, concise, no more than 5 sentences)'}. 
...
{'Output format:\nSUBJECT: [subject line]\n\n[email body]' if outreach_type == 'email' else '[WhatsApp message only — no subject]'}""",

    "emailFinder": lambda candName, context: f"""You are a recruitment research specialist. Based on the candidate information below, generate the most likely professional email addresses they might use.
...
Before the JSON, briefly explain your reasoning for each suggestion (2 sentences max each).""",
    ...
    "updateCVFromAnswers": lambda originalCV, questionnaire, answers: f"""You are a professional CV writer. A recruiter sent a candidate these clarification questions and received answers. Your job is to incorporate the candidate's answers into their CV to produce an improved, more complete version.
...
Output the complete updated CV text only. No commentary before or after.""",
    ...
    "reEvalSummary": lambda oldScore, newScore, oldEval, newEval, candName: f"""You are comparing two evaluations of the same candidate, {candName}, after their CV was updated with additional information.

Previous score: {oldScore}/100
New score: {newScore}/100
Score change: {'+' if newScore - oldScore > 0 else ''}{newScore - oldScore} points

Previous evaluation summary:
{oldEval[:800] if oldEval else 'N/A'}

New evaluation summary:
{newEval[:800] if newEval else 'N/A'}
