...
import { Btn, Card, SectionHeader, Label, Spinner } from '../components/UI.jsx';
...
import { PROVIDERS, PROVIDER_LIST, callAI } from '../lib/api.js';
...
export default function SettingsPage({ user, onUserUpdate, notify }) {
  ...
  const testConnection = async () => {
    if (!apiKey) { notify('Enter an API key first.', 'err'); return; }
    setTesting(true); setTestResult(null);
    try {
      let got = '';
      await callAI({ provider, apiKey, model }, 'Reply with exactly: OK', (t) => { got = t; });
      setTestResult('ok');
    } catch (e) {
      setTestResult('err');
    ...
    <div style={{ maxWidth:620 }}>
      ...
      <Card raised style={{ marginBottom:20 }}>
        ...
        <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:16 }}>
          ...
          {apiKey && (
            <Btn v="secondary" onClick={testConnection} disabled={testing}>
              {testing ? <><Spinner size={14}/>Testing…</> : <><Icon n="zap" size={14}/>Test Connection</>}
            </Btn>
          )}
        ...
        {/* Test result */}
        {testResult === 'ok' && (
          <div style={{ background:'#f0fdf4', border:'1.5px solid #bbf7d0', borderRadius:9, padding:'9px 14px', marginBottom:14, display:'flex', alignItems:'center', gap:9, fontSize:13, color:'#15803d', fontWeight:600 }}>
            <Icon n="check" size={16} color="#16a34a"/>Connection successful — {PROVIDERS[provider]?.name} is responding correctly.
          </div>
        )}
        {testResult === 'err' && (
          <div style={{ background:'#fff1f2', border:'1.5px solid #fecdd3', borderRadius:9, padding:'9px 14px', marginBottom:14, display:'flex', alignItems:'center', gap:9, fontSize:13, color:'#be123c', fontWeight:600 }}>
            <Icon n="warn" size={16} color="#be123c"/>Connection failed — check your API key and try again.
          </div>
        )}
